"""File helpers for ll_categorizer: data directory, atomic writes, rules overrides."""
...
def get_data_dir(base_dir: str | Path | None = None) -> Path:
    ...
    If ``base_dir`` is omitted, uses the ``LL_CATEGORIZER_DATA_DIR`` environment
    variable when set; otherwise ``./ll_categorizer_data`` under the current
    working directory.
    """
    if base_dir is not None:
        root = Path(base_dir)
    else:
        env = (os.environ.get("LL_CATEGORIZER_DATA_DIR") or "").strip()
        if env:
            root = Path(env).expanduser()
        else:
            root = Path.cwd() / "ll_categorizer_data"
...
def get_rules_overrides_path(base_dir: str | Path | None = None) -> Path:
    """JSON file of rule overrides merged on top of built-in defaults."""
    return get_data_dir(base_dir) / "rules_overrides.json"


def get_report_path(base_dir: str | Path | None = None) -> Path:
    return get_data_dir(base_dir) / "categorizer_report.txt"
...
def load_merged_category_rules(
    defaults: dict[str, CategoryRule],
    base_dir: str | Path | None = None,
) -> dict[str, CategoryRule]:
    """Return a copy of ``defaults`` merged with ``rules_overrides.json`` if present."""
    merged: dict[str, CategoryRule] = {
        k: {"category": v["category"], "subcategory": v["subcategory"]} for k, v in defaults.items()
    }
    path = get_rules_overrides_path(base_dir)
    custom = load_json(path, default={})
    ...
    for key, raw in custom.items():
        ...
        if isinstance(cat, str) and isinstance(sub, str):
            merged[str(key)] = {"category": cat, "subcategory": sub}
    return merged


def save_rules_overrides(
    current_rules: dict[str, CategoryRule],
    defaults: dict[str, CategoryRule],
    base_dir: str | Path | None = None,
) -> Path:
    """Write only entries that differ from ``defaults`` to ``rules_overrides.json``."""
    overrides: dict[str, CategoryRule] = {}
    for key, value in current_rules.items():
        default_entry = defaults.get(key)
        if default_entry != value:
            overrides[key] = value
    path = get_rules_overrides_path(base_dir)
    save_json(cast(dict[str, Any], overrides), path)
    return path
