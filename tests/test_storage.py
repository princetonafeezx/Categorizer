...
def test_get_data_dir_env_override(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("LL_CATEGORIZER_DATA_DIR", str(data))
...
def test_get_data_dir_explicit_base(tmp_path, monkeypatch) -> None:
    monkeypatch.delenv("LL_CATEGORIZER_DATA_DIR", raising=False)
...
def test_rules_overrides_merge_and_round_trip(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("LL_CATEGORIZER_DATA_DIR", str(tmp_path))
    defaults: dict[str, CategoryRule] = {
        "alpha": {"category": "A", "subcategory": "a"},
    }
    merged = storage.load_merged_category_rules(defaults)
    assert merged == defaults
    merged["beta"] = {"category": "B", "subcategory": "b"}
    storage.save_rules_overrides(merged, defaults)
    reloaded = storage.load_merged_category_rules(defaults)
    assert reloaded["alpha"] == {"category": "A", "subcategory": "a"}
    assert reloaded["beta"] == {"category": "B", "subcategory": "b"}


def test_rules_overrides_modified_default_persists(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("LL_CATEGORIZER_DATA_DIR", str(tmp_path))
    defaults: dict[str, CategoryRule] = {
        "alpha": {"category": "A", "subcategory": "a"},
    }
    current = storage.load_merged_category_rules(defaults)
    current["alpha"] = {"category": "A", "subcategory": "changed"}
    storage.save_rules_overrides(current, defaults)
    reloaded = storage.load_merged_category_rules(defaults)
    assert reloaded["alpha"]["subcategory"] == "changed"
